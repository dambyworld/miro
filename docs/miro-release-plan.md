...
- `bun` 스크립트(`scripts/release.ts`)로 자동화:
  - 아티팩트 다운로드
...
```
scripts/
  release.ts       # bun 실행 릴리스 자동화 스크립트
...
Tap 포뮬라 갱신은 `scripts/release.ts`가 릴리스 후 자동으로 PR을 생성한다.
